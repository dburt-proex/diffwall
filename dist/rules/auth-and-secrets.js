...
export const authAndSecretsRule = (files) => {
    const findings = [];
    const authFiles = files.map((file) => file.path).filter((path) => authPathPattern.test(path));
    if (authFiles.length > 0) {
        findings.push({ ruleId: "auth-sensitive-path", severity: "high", score: 25, message: "Authentication, authorization, billing, or security-sensitive files changed", files: authFiles });
    }
    ...
    if (secretEvidence.length > 0) {
        findings.push({ ruleId: "secret-like-string-added", severity: "critical", score: 100, message: "Secret-like credential or private key added", files: [...new Set(secretFiles)], evidence: secretEvidence.slice(0, 5), halt: true });
    }
    return findings;
};
