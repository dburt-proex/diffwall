...
function routeMessage(route) {
    if (route === "ALLOW")
        return "This PR is low risk under the current policy. Normal CI and review rules still apply.";
    if (route === "REVIEW")
        return "This PR should receive human review before merging.";
    return "This PR triggered a HALT condition and should not merge until the finding is fixed or explicitly overridden.";
}
function reviewFocus(result) {
    if (result.findings.length === 0)
        return ["- Standard correctness review"];
    return result.findings.slice(0, 6).map((finding) => `- ${finding.message}`);
}
function ownerSuggestions(result) {
    ...
    for (const match of matches)
        lines.push(`| ${match.file} | ${match.owners.join(", ")} |`);
    return lines;
}
