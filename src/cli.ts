...
function parseArgs(args: string[]): CliOptions {
  ...
  for (let index = 1; index < args.length; index += 1) {
    ...
    if (arg === "--base") options.base = takeValue(arg, next, () => index += 1);
    else if (arg === "--head") options.head = takeValue(arg, next, () => index += 1);
    else if (arg === "--staged") options.staged = true;
    else if (arg === "--diff") options.diff = takeValue(arg, next, () => index += 1);
